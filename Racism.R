...
##   a dictionary of slur words for 5 racial/ethic groups. These groups      ##
##   are White, Black, Hispanic, Asian, and Jewish. With this list we        ##
##   conduct a dictionary based search of tweets from 8 November 2018        ##
##   through 18 November 2018. These Tweets will then be scrubbed for        ##
##   three most common slurs per group. Those Tweets will then be            ##
##   analyized for a toxicity rating through Google's Racial Toxicity API.   ##
##   Once the most toxic tweets (those above a threshold of 60% likeliness)  ##
##   we conducted numerous anlyitical methods to include time series, topic  ##
##   modeling, text networks, frequency modeling, word clouds, and soical    ##
##   network analysis.                                                       ##
##                                                                           ##
###############################################################################
###############################################################################
...
#                                                                             #
#               We utilized the RTweet package in R in order to search        #
#               tweets in order to pull instances of hate speech. By doing    #
#               so we realized that the twitter search algorithm pulled       #
#               more than just the content of the tweet, which required       #
#               us to do additional text clearning later.                     #
#                                                                             #
###############################################################################
...
#               higher for our anaylsis                                       #
#                                                                             #
###############################################################################

labels                           <- read.csv("/Users/emmasun/labeltweets.txt", 
                                             sep = ' ', 
...
# select toxic tweets
alltweetsdegotslurtoxic          <- lltweetsdegotslur %>%
  filter(toxicity>0.6)
...
# white racist tweets on 11/07 and 11/06
alltweetsdegotslurwhite$time   <- as.Date(alltweetsdegotslurwhite$created_at,
                                         format = "%y-%m-%d")
...
# black racist tweets on 11/14
alltweetsdegotslurblack        <- alltweetsdegotslur[alltweetsdegotslur$racelabel == "black",]
alltweetsdegotslurblack$time   <- as.Date(alltweetsdegotslurblack$created_at, 
                                          format = "%y-%m-%d")
...
# jewish racist tweets on 11/14
alltweetsdegotslurjewish       <- alltweetsdegotslur[alltweetsdegotslur$racelabel == "jewish",]
alltweetsdegotslurjewish$time  <- as.Date(alltweetsdegotslurjewish$created_at, 
                                          format = "%y-%m-%d")
...
# read in selected slur words. I further filtered Joe's selection. 
# Filter out some ambiguous words like banana. 
slurwords         <- read.csv("file:///C:/Users/Joseph/Documents/R/RSDB hisp.csv")
...
###############################################################################
#                                                                             #
#               Determining the most common slurs against Jewsih              #
#                                                                             #
###############################################################################
...
### Mapping location of the tweets
...
### Violin Plot determining the density of toxicity tweets against each race
...
### Counting top words for all races
...
slurvocabday1        <- data.frame(vocab = character(nrow(top3slurs)), 
                                   count = numeric(nrow(top3slurs)), 
                                   stringsAsFactors = FALSE)
i=1
...
# Barplot
slurvocabday1$group <- c("white","white","white",
...
ggplot(selectedslurvocabday1, 
       aes(reorder(vocab, -count), 
           count, fill = factor(group))) + 
  geom_bar(stat = "identity")+
  labs(x= "slurvocab", 
       y= "count",
  ...
  scale_fill_manual("legend", values = c("white" = "royalblue4",
                                         "black" = "#404040", 
                                         "asian" = "#ca0020",
                                         "hispanic" = "#bababa", 
                                         "jewish" = "pink3"))
...
#what words are most closely related to gringo
normalized_prob %>% 
  filter(word1 == "gringo") %>%
  arrange(-p_together)

#what words are most closely related to redneck
normalized_prob %>% 
  filter(word1 == "redneck") %>%
  arrange(-p_together)

#what words are most closely related to zionist
normalized_prob %>% 
  filter(word1 == "zionist") %>%
  arrange(-p_together)

#what words are most closely related to coon
normalized_prob %>% 
  filter(word1 == "coon") %>%
  arrange(-p_together)

#what words are most closely related to chink
normalized_prob %>% 
  filter(word1 == "chink") %>%
  arrange(-p_together)
...
###############################################################################
#                                                                             #
#               WORD EMBEDDING - Slurs against Blacks                         #
#                                                                             #
###############################################################################
...
###############################################################################
#                                                                             #
#               WORD EMBEDDING - Slurs against Whites                         #
#                                                                             #
###############################################################################
...
###############################################################################
#                                                                             #
#               WORD EMBEDDING - Slurs against Jews                           #
#                                                                             #
###############################################################################
...
###############################################################################
#                                                                             #
#               WORD EMBEDDING - Slurs against Asians                         #
#                                                                             #
###############################################################################
...
###############################################################################
#                                                                             #
#               WORD EMBEDDING - Slurs against Hispanics                      #
#                                                                             #
###############################################################################
...
#               MACHINE LEARNING                                              #
#                                                                             #
#               Addtional attempt at utilizing machine learning prior         #
#               of google's API for toxicity in NLP                           #
#                                                                             #
